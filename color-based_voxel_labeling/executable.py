...
def main():
    ...
    lastUpdateTime = glfw.get_time()  # Use GLFW's timing function for consistency
    iterator = 0  # Start the iterator at 0
    n = 1
    animationStartTime = glfw.get_time() + 15
    while not glfw.window_should_close(window):
        current_time = glfw.get_time()  # Current time from GLFW, consistent with lastUpdateTime
        delta_time = current_time - last_time
        last_time = current_time

        # Check if n seconds have passed since the last update
        if current_time >= animationStartTime:
            if current_time - lastUpdateTime >= n:
                print(iterator)
                lastUpdateTime = current_time  # Reset the last update time
                
                # Update the voxel positions with the current iterator value
                positions, colors = set_voxel_positions(config['world_width'], config['world_height'], config['world_width'], iterator)
                cube.set_multiple_positions(positions, colors)
                
                iterator += 1  # Increment the iterator

        # The rest of your main loop code follows
